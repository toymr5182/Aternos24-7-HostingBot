...
let botState = {
  ...
  errors: [],
  wasThrottled: false
  lastServerMessage: 'No messages yet'
};
bot.on('message', (jsonMsg) => {
  const msg = jsonMsg.toString().trim();
  if (msg) {
    botState.lastServerMessage = msg;
  }
});
...
app.get('/', (req, res) => {
  ...
  <input id="cmdInput" placeholder="/login password"
    style="width:100%;padding:12px;border-radius:10px;border:none;margin-bottom:10px;box-sizing:border-box;" />
...
<div class="card">
  <div class="label">LAST SERVER MESSAGE</div>
  <div>${botState.lastServerMessage}</div>
</div>
...
app.post('/command', (req, res) => {
  ...
  if (!bot || !botState.connected) {
    return res.status(400).json({ ok: false, error: 'Bot not connected' });
  }
...
if (message.startsWith('!cmd ')) {
  ...
  if (cmd) bot.chat(cmd.startsWith('/') ? cmd : '/' + cmd);
}
